...
\label{eq:E(IRR)}
    ...
    (I) = \frac{\text{exp}(\alpha+\beta_{1}+0+0)}{\text{exp}(\alpha+0+0+0)} = \text{exp}(\alpha + \beta_{1} - \alpha) = \text{exp}(\beta_{1})\\[0.5cm]
    \begin{array}{lcll}
        (II) = \frac{\displaystyle \text{exp}(\alpha + \beta_{1} + \sum_{k=1}^{4} \gamma_{k}\mathbbm{1}_{j=k} + \displaystyle \sum_{k=1}^{4} \delta_{1k}\mathbbm{1}_{j=k})}{ \displaystyle \text{exp}(\alpha + 0 + \sum_{k=1}^{4} \gamma_{k}\mathbbm{1}_{j=k} + 0)}\\[0.2cm]
        = \displaystyle \text{exp}(\alpha + \beta_{1} + \sum_{k=1}^{4} \gamma_{k}\mathbbm{1}_{j=k} + \sum_{k=1}^{4} \delta_{1k}\mathbbm{1}_{j=k} - \alpha - \displaystyle \sum_{k=1}^{4} \gamma_{k}\mathbbm{1}_{j=k})\\[0.2cm]
...
Hence, 
\begin{equation}
        \mathbb{E}(IRR_{j}) = \begin{cases} e^{\beta_{1}}, & \mbox{if $j = 0$,} \\ e^{\beta_{1} + \delta_{1j}}, & \mbox{if $j \not= 0$.} \end{cases}\\
\end{equation}
\QEDB
