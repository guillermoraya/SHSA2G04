...
@
  \begin{enumerate}
    ...
    \begin{itemize}
      \item Observing the incidence rate for the age range of 45 to 54 years old, we can see that there were more cases of coronary deaths among the exposed group (\Sexpr{Ir_45smoker} cases a year per 1000 people, to be specific) than among the unexposed or "control" group (which suffered \Sexpr{Ir_45Nonsmoker} cases a year per 1000 people). Comparing the two figures, we see that the smokers had an incidence rate (or number of cases per unit of people and time) \Sexpr{IRR_45} as big as non-smokers, a figure that seems to indicate that smokers in this age group had a bigger chance of suffering from coronary death. In order to check if these results are significant, we have performed a Wald test (the null hypothesis being "the incidence rato of exposed and non-exposed groups are equivalent"), which yielded a p-value of \Sexpr{pValue45}. Having received a p-value lower than 0.05, we believe that the null hypothesis can be rejected: smoking seems to affect the incidence rate of coronary death among 45 to 54 year-olds.
    \end{itemize}
  \end{enumerate}
...
@
  ...
  \end{enumerate}
<<IRR_plot,fig.width=7, fig.height=4, fig.cap='\\label{fig:IRR_plot}Plots of the incidence rate ratios depending on the age range.The graph on the left is in linear scale, while the graph on the right is on logarithmic scale.'>>=
...
\begin{itemize}
    \item $\widehat{IRR}_{35-44}$ = 
    \item $\widehat{IRR}_{45-54}$ = 
    \item $\widehat{IRR}_{55-64}$ = 
    \item $\widehat{IRR}_{65-74}$ = 
    \item $\widehat{IRR}_{75-84}$ = 
\end{itemize}
