...
\midrule
\Sexpr{Data$age[1]} & \Sexpr{Data$personYears[1]} & \Sexpr{Data$personYears[6]}  & \Sexpr{Data$deaths[1]} & \Sexpr{Data$deaths[6]} & \Sexpr{Ir_Nonsmokers[1]} & \Sexpr{Ir_smokers[1]} & \Sexpr{IRR[1]}\\
\Sexpr{Data$age[2]} & \Sexpr{Data$personYears[2]} & \Sexpr{Data$personYears[7]}  & \Sexpr{Data$deaths[2]} & \Sexpr{Data$deaths[7]} & \Sexpr{Ir_Nonsmokers[2]} & \Sexpr{Ir_smokers[2]} & \Sexpr{IRR[2]}\\
\Sexpr{Data$age[3]} & \Sexpr{Data$personYears[3]} & \Sexpr{Data$personYears[8]}  & \Sexpr{Data$deaths[3]} & \Sexpr{Data$deaths[8]} & \Sexpr{Ir_Nonsmokers[3]} & \Sexpr{Ir_smokers[3]} & \Sexpr{IRR[3]}\\
\Sexpr{Data$age[4]} & \Sexpr{Data$personYears[4]} & \Sexpr{Data$personYears[9]}  & \Sexpr{Data$deaths[4]} & \Sexpr{Data$deaths[9]} & \Sexpr{Ir_Nonsmokers[4]} & \Sexpr{Ir_smokers[4]} & \Sexpr{IRR[4]}\\
\Sexpr{Data$age[5]} & \Sexpr{Data$personYears[5]} & \Sexpr{Data$personYears[10]} & \Sexpr{Data$deaths[5]} & \Sexpr{Data$deaths[10]} & \Sexpr{Ir_Nonsmokers[5]} & \Sexpr{Ir_smokers[5]} & \Sexpr{IRR[5]}\\
\bottomrule
\end{tabular}
\caption{Data on coronary death rates including incidence rate depending on somke status and incidence rate ratio.}
\label{tab:table2}
\end{table}
  
  \item For age group 45-54, complete the following sentences with numbers and proper units \textbf{(using} \verb+\Sexpr+ \textbf{in your \Robject{Rnw} document)}:
...
@
  ...
  \end{enumerate}
<<IRR_plot,fig.width=7, fig.height=4, fig.cap='\\label{fig:IRR_plot}Plots of the incidence rate ratios depending on the age range.The graph on the left is in linear scale, while the graph on the right is on logarithmic scale.', echo=FALSE>>=
...
\begin{itemize}
    ...
    \begin{fleqn}
        \begin{alignat*}{3}
            I_{r_{ij}} = \frac{I_{ij}}{\Delta t_{ij}}, &\qquad I_{ij} \sim \text{Pois}(\lambda_{ij}), &\qquad \Delta t_{ij} \text{ is the follow-up;}
        \end{alignat*}
    \end{fleqn}

    \item Model for the mean:
    \begin{flalign}
    \label{eq:model}
        &\text{log} \left(\frac{\lambda_{ij}}{\Delta t_{ij}} \right) = L(E_{i},A_{j}) \text{ (the linear predictor)}&
    \end{flalign}
    \begin{flalign*}
        L(E_{i},A_{j}) &= \alpha + \beta_{i}E_{i} + \gamma_{j}A_{j} + \delta_{ij}E{i}A{j} &\\
        &= \alpha + &\\
        &+ \beta_{1}\mathbbm{1}_{i=1} + &\\
        &+ \gamma_{1}\mathbbm{1}_{j=1} + \gamma_{2}\mathbbm{1}_{i=2} + \gamma_{3}\mathbbm{1}_{i=3} + \gamma_{4}\mathbbm{1}_{i=4} + &\\
        &+ \delta_{11}\mathbbm{1}_{i=1}\mathbbm{1}_{j=1} + \delta_{12}\mathbbm{1}_{i=1}\mathbbm{1}_{j=2} + \delta_{13}\mathbbm{1}_{i=1}\mathbbm{1}_{j=3} + \delta_{14}\mathbbm{1}_{i=1}\mathbbm{1}_{j=4}, &
    \end{flalign*}
\end{itemize}
where
\begin{equation*}
    \mathbbm{1}_{x=a} = \begin{cases} 1, & \mbox{if $x = a$,} \\ 0, & \mbox{if $x \not= a$.} \end{cases}
\end{equation*}
...
\begin{enumerate}
  \item Note that the logarithm function in model \eqref{eq:model} implies that we are assuming that the incidence rate varies exponentially with age. Is it         consistent with Figure 1? Why?
...
\end{equation}
\QEDB
\item According to Figure 1, explain what sign (i.e. positive, negative or zero) do you
